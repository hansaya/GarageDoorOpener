...
void CCS811::begin()
{
    ...
    if (!ccs.begin(0x5A))
    {
        DEBUG_PRINTLN("Failed to start CCS811 sensor! Reseting the sensor..");
        reset();
        if (!ccs.begin(0x5A))
        {
...
void CCS811::loop()
{
    ...
    if (currentMillis - sensorDataPublishPeriod >= 60 * 1000)
    {
        ...
        {
            if (ccs.checkError())
                DEBUG_PRINTLN("CCS811 internal error! Reseting the sensor..");
            else
                DEBUG_PRINTLN("CCS811 not working properly! Reseting the sensor..");
